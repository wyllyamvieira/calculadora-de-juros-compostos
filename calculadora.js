...
.on('click', '.simulaUm', function () {
    ...
    let montante = parseFloat($('.montante').val() * 1),
        aporte = parseFloat($('.aporte').val() * 1),
        prazo = parseFloat($('.prazo').val() * 1),
        taxa = parseFloat($('.taxa').val()) / 100;
    ...
    $('.apIn').text(Number(montante).praPessoa());
    $('.apOut').text(Number(montante + aporte * prazo).praPessoa());
    $('.apJu').text(Number(jurosPush).praPessoa());
    $('.apTot').text(Number(acuPush).praPessoa());
    console.log(dados);
    console.log(acumulado);
    $('#printfinal').find('tbody').html(acumulado);
});
...
Calculator.Calculate = function () {
    ...
    e = $("input[name=hdnDimensionsWallIndex]");
    ...
    o = $("input[name=hdnDimensionsDoorWindowIndex]");
...
Calculator.GetTypePaints_End = function (n) {
    ...
    for (n = n || {
        TypePaints: []
    }, i = "", n.TypePaints.length > 1 && (i += '<option value="-1">Selecione o produto<\/option>'), t = 0; t < n.TypePaints.length; t++) {
        i += String.Format('<option value="{0}">{1}<\/option>', n.TypePaints[t].Id, n.TypePaints[t].Name);
    }
